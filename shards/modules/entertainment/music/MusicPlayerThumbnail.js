...
/**
 * @description Generates a thumbnail image for now playing music
*/
class MusicPlayerThumbnail extends IrisModule {
    ...
    */
     ...
     * @return {Promise<Canvas>} The circle canvas
    */
    static async getCircle(url) {
        const canvas = createCanvas(512, 512);
        const ctx = canvas.getContext("2d");

        const webpImage = await axios.get(url, {responseType: "arraybuffer"}).catch((error) => {
            console.log("error occured"); console.log(error);
        });
        if (!webpImage || !webpImage.data) {
            return;
        }

        let pngImage = await sharp(webpImage.data);
        const pngImageData = await pngImage.metadata();

        let resize;
        if (pngImageData.width >= pngImageData.height) {
            resize = {height: 600};
        } else {
            resize = {width: 600};
        }
        ...
        const radius = pngImage.height / 2;
     ...
     * @return {String} The duration in h:mm:ss
    */
     ...
     * @return {Canvas} The now playing canvas
    */
    static async getNowPlaying(url, title, artist, duration) {
        ...
        const circle = await MusicPlayerThumbnail.getCircle(url);
