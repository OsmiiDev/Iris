...
class MusicPlayerPlaylists extends IrisModule {
     ...
     * @returns {Object} The playlists for the user
    */
    static getPlaylists(user) {
        ...
        let playlistFormat = {
            name: "title", url: "https:/youtube.com/watch?"
        }
    }
    ...
    */
    static async viewPlaylists(interaction) {
        let playlists = MusicPlayerPlaylists.getPlaylists(interaction.user);

        let embed = MessageUtils.generateEmbed("Playlists",
        ...
        if (playlists) {
            let playlistMenu = new MessageSelectMenu().setCustomId(`723f421f55f0-${interaction.member.id}`)
                .setPlaceholder("Select a playlist")

            let playlistSelector = new MessageActionRow().addComponents(playlistMenu);
        ...
        let playlistCreateButton = new MessageButton()
    ...
    */
    async selectPlaylist(interaction) {
        if (!interaction.isSelectMenu() || !interaction.customId.startsWith("723f421f55f0-")) { return; }
        if (!interaction.guild || !interaction.member || !PermissionUtils.hasPermission(interaction.member, "MUSIC_PLAYER_PLAYLISTS")) { return; }
        if (!interaction.guild.me.permissions.has("ADMINISTRATOR")) { return; }

        if (interaction.member.id !== interaction.customId.split("-")[1]) { return interaction.reply({ embeds: [MessageUtils.generateErrorEmbed("You can't do that.")], ephemeral: true }); }
        if (!interaction.values || !interaction.values[0]) { return; };

        console.log(interaction.values[0].split("-")[0]);
        let data = MusicPlayerPlaylists.getPlaylists(interaction.user)[interaction.values[0].split("-")[0] * 1];
        if (!data) { return; }

        let embed = MessageUtils.generateEmbed(`Playlist: ${data.name}`, data.description, "#4466DD", interaction.user).setFooter({ text: "Iris Music" }).setTimestamp();
