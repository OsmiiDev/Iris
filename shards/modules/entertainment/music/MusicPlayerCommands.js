const { CommandInteraction, VoiceState, MessageEmbed, MessageButton, MessageActionRow } = require("discord.js");
...
class MusicPlayerCommands extends IrisModule {
     ...
     * @param {String} video The URL or ID of the video (https://youtube.com/watch?v=ID, etc.)
    ...
    */
    static async play(interaction, song, queue) {
        ...
        if (song.match(/^(http(s)?:\/\/)?(www\.)?youtube\.com\/watch\?v=([a-zA-Z0-9-_]){11}/g)) { url = song; }
        else if (song.match(/^(http(s)?:\/\/)?(www\.)?youtu\.be\/([a-zA-Z0-9-_]){11}/g)) { url = song; }
        else if (song.match(/^(http(s)?:\/\/)?(www\.)?youtube\.com\/v\/([a-zA-Z0-9-_]){11}/g)) { url = song; }
        else if (song.match(/^(http(s)?:\/\/)?(www\.)?open\.spotify\.com\/track\/([a-zA-Z0-9-_]){22}/g)) { url = song; }
        ...
        let whitelist = DataUtils.getConfig(interaction.guild).modules.entertainment.music['whitelisted-voice-channels'];
        ...
        let history = DataUtils.readUser(interaction.user, "entertainment/music/history", {});
        if (!history[interaction.guild.id]) { history[interaction.guild.id] = []; }
        ...
        let videoInformation = Object.fromEntries(Object.entries(information).filter((value, index) => properties.includes(value[0])));
        ...
        let buttons = new MessageActionRow().addComponents(viewQueueButton);
        ...
        if (youtubeResults) {
            ...
            interaction.reply({
                embeds: [
                    MessageUtils.generateEmbed(`Added to queue: ${title}`,
                        `[Watch on YouTube](${url}) | [Subscribe to creator](https://www.youtube.com/channel/${information.channelId}?sub_confirmation=1)\n
                    **Uploaded by:** ${information.author.name.endsWith(" - Topic") ? information.author.name.split(" - ")[0] : information.author.name}
                    **Uploaded on:** ${new Date(information.uploadDate).toLocaleDateString()}${information.category === "Music" ? "" : "\n\n<:Iris_Information:982364440651522088> This video is not tagged as Music."}`,
                        "#4466DD", interaction.user).setThumbnail(information.thumbnails[information.thumbnails.length - 1].url).setFooter({ text: "Iris Music" }).setTimestamp()
                ],
                components: [buttons]
            });
    ...
    */
    static async loop(interaction) {
        ...
        let loop = MusicPlayer.toggleLoop(interaction.guild);

        if (loop) {
            let queueEmbed = MessageUtils.generateEmbed("Now looping the current queue", "When the queue empties, it will replay from the beginning.", "#4466DD", interaction.user).setFooter({ text: "Iris Music" }).setTimestamp();
